"""
Interactive orchestrator module for Amplifier.
Extends streaming orchestrator with mid-execution message injection.
"""
...
async def mount(coordinator: ModuleCoordinator, config: dict[str, Any] | None = None):
    """Mount the interactive orchestrator module."""
    config = config or {}
    ...
    coordinator.register_contributor(
        ...
        lambda: [
            ...
            "injection:applied",  # When injected messages are applied
        ],
    )
    ...
    logger.info("Mounted InteractiveOrchestrator with injection support")
    return


class InteractiveOrchestrator:
    """
    Interactive implementation of the agent loop.
    Extends streaming orchestrator with mid-execution message injection.
    ...
    def inject_message(self, content: str) -> None:
        ...
        logger.info("Message injected into execution queue: %s", content[:80])

    async def _drain_injection_queue(self, context, hooks: HookRegistry) -> bool:
    ...
    async def execute(
        ...
        on_progress: Any | None = None,  # Callable[[str, dict], Awaitable[None]] | None
    ) -> str:
        """
        ...
        """
        # Store progress callback for this execution
        self._on_progress = on_progress
        # Clear any stale messages from previous executions
        while not self._injection_queue.empty():
            try:
    ...
    async def _execute_stream(
        ...
        while self.max_iterations == -1 or iteration < self.max_iterations:
            ...
            if self._on_progress:
                try:
                    await self._on_progress("thinking", {"iteration": iteration})
                except Exception:
                    pass
            ...
            if hasattr(provider, "stream"):
                ...
                if await self._has_pending_tools(context):
                    ...
                    # INJECTION POINT 2 (streaming): Check before breaking
                ...
                try:
                    ...
                    if not tool_calls:
                        ...
                        # INJECTION POINT 2: Check for messages before committing to "done"
                        if not self._injection_queue.empty():
                            injected = await self._drain_injection_queue(context, hooks)
                            if injected:
                                logger.info(
